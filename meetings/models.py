...
class Meeting(models.Model):
    ...
    def is_upcoming(self):
        return self.start_time > timezone.now() and self.status == self.STATUS_SCHEDULED
...
class Participant(models.Model):
    ...
    def __str__(self):
        return f"{self.email} → {self.meeting.title} [{self.status}]"
...
class MeetingNotification(models.Model):
    ...
    def __str__(self):
        return f"[{self.notification_type}] → {self.email} | {self.meeting.title}"
